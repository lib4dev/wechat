...
func NewContext(srv IAccessToken) *Context {
	return NewContextByClient(srv, nil)
}

// NewContextByClient 创建一个新的 Context.
//  如果 clt == nil 则默认用 util.DefaultHttpClient
func NewContextByClient(srv IAccessToken, clt *http.Client) *Context {
	if srv == nil {
		panic("nil AccessToken")
	}
	if clt == nil {
...
//          ...
//      }
func (clt *Context) GetJSON(incompleteURL string, response interface{}) (err error) {
...
//          ...
//      }
func (clt *Context) PostJSON(incompleteURL string, request interface{}, response interface{}) (err error) {
	ErrorStructValue, ErrorErrCodeValue := checkResponse(response)
